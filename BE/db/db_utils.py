...
def get_connection():
    """Legacy helper — returns a raw connection from the pool.
    Caller must call pool.putconn(conn) when done, or use _PooledConnection context manager.
    Kept for backwards-compat; prefer _PooledConnection in new code."""
...
def get_conversations_by_user(user_id):
    ...
    try:
        ...
        cur.execute(
            """
            SELECT id, user_id, name, status, created_at FROM conversations
            WHERE user_id = %s
            ORDER BY id DESC
            """,
            (user_id,)
