...
def create_conversation(user_id, name):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO conversations (user_id, name, status) VALUES (%s, %s, %s) RETURNING id",
        (user_id, name, 1)
    )
    result = cur.fetchone()
...
def add_message(conversation_id, content, created_by, role='user'):
    ...
    cur = conn.cursor()
    cur.execute(
        "INSERT INTO messages (conversation_id, status, content, created_by, role) VALUES (%s, %s, %s, %s, %s) RETURNING id",
        (conversation_id, 1, content, created_by, role)
    )
    result = cur.fetchone()
